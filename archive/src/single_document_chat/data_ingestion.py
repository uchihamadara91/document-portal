...
class SingleDocIngestor:
    def __init__(self, data_dir: str = "data/single_document_chat", faiss_dir: str = "faiss_index"):
        try:
            ...
            self.data_dir.mkdir(parents=True, exist_ok=True)  # This till here is Data Versioning
            self.faiss_dir = Path(faiss_dir)
            self.faiss_dir.mkdir(parents=True, exist_ok=True) # Vector DB folder
    ...
    def ingest_files(self,uploaded_files):
        try:
            ...
            for uploaded_file in uploaded_files:
                unique_filename = f"session_{datetime.now(timezone.utc).strftime('%d%m%Y_%H%M%S')}_{uuid.uuid4().hex[:8]}.pdf"
            ...
            raise DocumentPortalException("Error in File Ingestion",sys)
    ...
    def _create_retriever(self, documents):
        try:
            ...
            self.log.error("Retriever Creation Failed Failed", error=str(e))
